...
class ahnyKadishHut(ptResponder):
    ...
    def __init__(self):
        ...
        self.id = 5610
        self.version = 4
        PtDebugPrint("ahnyKadishHut: v{}".format(self.version), level=kWarningLevel)
    ...
    def OnFirstUpdate(self):
        try:
            ...
            PtDebugPrint("ahnyKadishHut.OnFirstUpdate(): Cannot find AhnySphere04 age SDL.", level=kErrorLevel)
        ...
        if self.boolWindows:
            PtDebugPrint("ahnyKadishHut.OnFirstUpdate(): Windows are open.", level=kWarningLevel)
            RespWindows.run(self.key, state="open", fastforward=1)
        else:
            PtDebugPrint("ahnyKadishHut.OnFirstUpdate(): Windows are closed.", level=kWarningLevel)
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):        
        if VARname == SDLWindows.value:
            ...
            self.boolWindows = ageSDL[SDLWindows.value][0]
            if self.boolWindows:
                PtDebugPrint("ahnyKadishHut.OnSDLNotify(): Windows will now open.", level=kWarningLevel)
                RespWindows.run(self.key, state="open")
            else:
                PtDebugPrint("ahnyKadishHut.OnSDLNotify(): Windows will now close.", level=kWarningLevel)
